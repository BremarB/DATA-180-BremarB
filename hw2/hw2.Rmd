...
```{r}
barplot(table(Cars93$Type),ylab="Frequency",xlab ="Type" ,cex.lab=1.2,col=c(2,3,4,5,6,7))
```
...
```{r}
pie(table(Cars93$Type), main="Type",col=c(2,3,4,5,6,7),cex=1.2)
```
...
hist(Horsepower, cex.lab=1.2,cex.axis=1.2, col="red")
```
...
```{r}
Horsepower<-Cars93$Horsepower
hist(Horsepower,cex.lab=1.2,cex.axis=1.2,col="red",breaks=seq(50,300,25))
...
```{r}
#The histogram using breaks is a more detailed version of the histogram and closely follows the suggested number of classes. It going by increments of 25 instead of 50 shows a more accurate representation of the data.
```
...
hist(Passengers, cex.lab=1.2,cex.axis=1.2, col="red")
#The data shown on the histogram doesn't show fully accurate discription of the data
```
...
```{r}
Passengers <-Cars93$Passengers
hist(Passengers, cex.lab=1.2,cex.axis=1.2, col="red",breaks = (9))
```
...
hist(Length, cex.lab=1.2,cex.axis=1.2, col="Green")
```
